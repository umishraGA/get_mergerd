...
class FunCategoriesScreen extends StatelessWidget {
  const FunCategoriesScreen({super.key});
          ...
          child: Obx(() {
            switch (controller.status.value) {
              ...
              // Show actual categories
                return GridView.builder(
                  itemCount: controller.categoriesFun.length,
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                    crossAxisCount: 2,
                    childAspectRatio: 0.9,
                  ...
                  itemBuilder: (context, index) {
                    var data = controller.categoriesFun[index];
                    return CategoryCard(
                      icon: data.icon ?? "",
                      type: "${data.type}",
                      // viewImage: true,
