...
class ReactionDisplayWidget extends StatefulWidget {
  ...
  /// Map of reaction types to lists of usernames who reacted
...
class _ReactionDisplayWidgetState extends State<ReactionDisplayWidget> {
  ...
  Widget _buildReactionsBar() {
    ...
    // First add the reactions with usernames
    widget.recentReactions.forEach((reaction, usernames) {
      if (usernames.isNotEmpty && totalDisplayed < maxReactionsToShow) {
        reactionWidgets.add(
          Container(
            height: 24,
            width: 24,
  ...
  String _buildReactionSummaryText() {
    ...
    if (widget.currentUserReaction != null && !allUsers.contains('You')) {
      allUsers.insert(0, 'You');
    }

    if (allUsers.isEmpty) {
      return '${widget.reactionCount} ${widget.reactionCount == 1 ? 'like' : 'likes'}';
    }

    if (allUsers.length == 1) {
      return '${allUsers[0]}${widget.reactionCount > 1 ? ' and ${widget.reactionCount - 1} others' : ''}';
    } else if (allUsers.length == 2) {
      return '${allUsers[0]}, ${allUsers[1]}${widget.reactionCount > 2 ? ' and ${widget.reactionCount - 2} others' : ''}';
    } else {
      return '${allUsers[0]}, ${allUsers[1]} and ${widget.reactionCount - 2} others';
    }
  }
}
...
class _ReactionListBottomSheetState extends State<ReactionListBottomSheet>
  ...
  Widget _getReactionIcon(ReactionType type, {double size = 18}) {
    switch (type) {
      case ReactionType.like:
  ...
  Widget _getFallbackReactionIcon(ReactionType type, {double size = 18}) {
  ...
  List<String> _getUsersForReaction(ReactionType? type) {
    if (type == null) {
      // All reactions
      Set<String> allUsers = {};
  ...
  List<String> _getFilteredUsers(List<String> users) {
  ...
  Widget _buildSearchResults() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildUserListItem(username, userReaction);
      },
    );
  }
      ...
      child: Column(
        ...
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: isSelected
                  ? _getReactionColor(reaction).withOpacity(0.1)
                  : Colors.transparent,
              shape: BoxShape.circle,
            ),
            child: Stack(
  ...
  Widget _buildUserListItem(String username, ReactionType? userReaction) {
    final isCurrentUser = username == 'You';
      ...
      child: ListTile(
        ...
        trailing: userReaction != null
            ? Container(
                ...
                height: 32,
                decoration: BoxDecoration(
                  color: _getReactionColor(userReaction).withOpacity(0.1),
                  shape: BoxShape.circle,
                ),
                child: Center(
                  child:
